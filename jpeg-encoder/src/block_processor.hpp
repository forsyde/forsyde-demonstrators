...
void color_conversion_func(tokens<color_conversion_out_t> &out, const tokens<bitmap_reader_out> &inp1)
{
  ...
  bitmap_reader_out input = get<0>(inp1);
  std::cout << "Processing block " << input.block_id << std::endl;


#pragma ForSyDe begin color_conversion_func

  auto         current_color_block = input.color_block;
  unsigned int current_width       = input.width;
  unsigned int current_height      = input.height;
