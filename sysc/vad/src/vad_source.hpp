/**********************************************************************
 * vad_source.hpp                                                  *
 *                                                                 *
 * Author:  Hosein Attarzadeh (shan2@kth.se)                       *
 *          adapted from KisTA: https://github.com/nandohca/kista  *
 *                                                                 *
 * Purpose: Provide input stimuli                                  *
 *                                                                 *
 * Usage:   The VAD example                                        *
 *                                                                 *
 * License: BSD3                                                   *
 *******************************************************************/

#ifndef VADSOURCE_HPP
#define VADSOURCE_HPP

#include <forsyde.hpp>
#include <iostream>

using namespace ForSyDe;
using namespace ForSyDe::SDF;

void vadSource_func(token_tuple<r_t,r_t,r_t,short,short,rc_t,short>& out,
		    const std::string& line)
{
  // Resize all the vectors to contain 1 element
  out = token_tuple<r_t,r_t,r_t,short,short,rc_t,short>({1,1,1,1,1,1,1});

  std::stringstream ss(line);
    
  // read r_h_1
  for (short& val: (std::get<0>(out.t))[0]) ss >> val;
  ss.ignore(1);   //ignore comma
    
  // read r_h_2
  (std::get<1>(out.t))[0] = (std::get<0>(out.t))[0];
  //~ for (short& val: (std::get<1>(out))[0])
  //~ ss >> val;
  //~ ss.ignore(1);   //ignore comma
    
  // read r_l
  for (short& val: (std::get<2>(out.t))[0]) ss >> val;
  ss.ignore(1);   //ignore comma
    
  // read scal_acf_1
  ss >> (std::get<3>(out.t))[0];
  ss.ignore(1);   //ignore comma
    
  // read scal_acf_1
  (std::get<4>(out.t))[0] = (std::get<3>(out.t))[0];
  //~ ss >> (std::get<4>(out))[0];
  //~ ss.ignore(1);   //ignore comma
    
  // read rc
  for (short& val: (std::get<5>(out.t))[0])  ss >> val;
  ss.ignore(1);   //ignore comma
    
  // read pitch
  ss >> (std::get<6>(out.t))[0];

}


#endif
