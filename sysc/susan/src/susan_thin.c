...
/**
 * Function for applying the thinning process onto the MCU block
 *
 * @param imgInput
               ...
               MCU_BLOCK *imgOutput, EdgeDirection *edgeDir)
{
  ...
  int l[9], centre, b01, b12, b21, b10, p1, p2, p3, p4, b00,
  ...
  /* EdgeDirection will be changed in susanThin,
   * copy original input edgeDirInput to edgeDir first
   */
  memcpy(edgeDir, edgeDirInput, sizeof(EdgeDirection));

  int *r = (int*) edgeSt->rBuffer;
  uchar *mid = edgeDir->midBuffer;

  /*Total dimensionas of the MCU block*/
  int x_size = imgInput->BL_WIDTH + imgInput->LEFT + imgInput->RIGHT;
  int y_size = imgInput->BL_HEIGHT + imgInput->TOP + imgInput->BOTTOM;

  for (i = imgInput->TOP; i < y_size - imgInput->BOTTOM; i++)
    for (j = imgInput->LEFT; j < x_size - imgInput->RIGHT; j++)
      if (mid[i * x_size + j] < 8) {
        centre = r[i * x_size + j];
        ...
        n = (*mp < 8) + (*(mp + 1) < 8) + (*(mp + 2) < 8)
            + (*(mp + x_size) < 8) + (*(mp + x_size + 2) < 8) + (*(mp + x_size
            + x_size) < 8) + (*(mp + x_size + x_size + 1) < 8) + (*(mp + x_size
            + x_size + 2) < 8);
        ...
        /* extension is only allowed a few times -
         * the value of mid is used to control this */
        ...
        if (n == 2) {
          ...
          if (((b00 + b02 + b20 + b22) == 2) && ((b00 | b22) & (b02 | b20))) {
            ...
            if (((float) r[(i + y) * x_size + j + x] / (float) centre) > 0.7) {
              if (((x == 0) && (mid[(i + (2 * y)) * x_size + j] > 7) && (mid[(i
                  + (2 * y)) * x_size + j - 1] > 7) && (mid[(i + (2 * y))
                  * x_size + j + 1] > 7)) || ((y == 0) && (mid[(i) * x_size + j
                  + (2 * x)] > 7) && (mid[(i + 1) * x_size + j + (2 * x)] > 7)
            ...
            if (((b01 + b12 + b21 + b10) == 2) && ((b10 | b12) & (b01 | b21))
                && ((b01 & ((mid[(i - 2) * x_size + j - 1] < 8) | (mid[(i - 2)
                    * x_size + j + 1] < 8))) | (b10 & ((mid[(i - 1) * x_size
                    + j - 2] < 8) | (mid[(i + 1) * x_size + j - 2] < 8)))
                    | (b12 & ((mid[(i - 1) * x_size + j + 2] < 8)
                        | (mid[(i + 1) * x_size + j + 2] < 8))) | (b21
                    & ((mid[(i + 2) * x_size + j - 1] < 8) | (mid[(i + 2)
                        * x_size + j + 1] < 8))))) {
